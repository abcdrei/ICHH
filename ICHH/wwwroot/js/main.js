...
(function ($) {
    ...
    // Scroll to hash anchor on page load (e.g. about.html#section-mission)
    // Needed because stack-item overflow-hidden blocks native anchor scrolling
    if (window.location.hash) {
        var hashTarget = window.location.hash;
        setTimeout(function () {
            var $target = $(hashTarget);
            if ($target.length) {
                var headerHeight = getHeaderHeight ? getHeaderHeight() : 0;
                $('html, body').animate({
